...
ARRAY_TASK_COUNT=${ARRAY_TASK_COUNT:-$((PROBLEMS_COUNT>1000 ? 1000 : PROBLEMS_COUNT))}
ARRAY=${ARRAY:-0-$((ARRAY_TASK_COUNT - 1))}
TIME_PER_TASK=${TIME_PER_TASK:-$((PROBLEMS_COUNT * (SOLVE_RUNS_PER_PROBLEM + 2) * (VAMPIRE_TIME_LIMIT + 10) / (ARRAY_TASK_COUNT * 60)))}
