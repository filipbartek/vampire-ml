...
class GCN(tf.keras.layers.Layer):
    def __init__(self, cfg, canonical_etypes, ntype_in_degrees, ntype_feat_sizes, output_ntypes=None, name='gcn',
        ...
        if isinstance(cfg.activation, str):
            activation = tf.keras.activations.deserialize(cfg.activation)

        ntypes = ntype_in_degrees.keys()

        if cfg.aggregate == 'concat':
            ntype_embedding_lengths = {ntype: in_degree * cfg.message_size for ntype, in_degree in
                                       ntype_in_degrees.items()}

            def aggregate_fn(tensors, dsttype):
                return tf.concat(tensors, axis=1)
        else:
            ntype_embedding_lengths = {ntype: cfg.message_size for ntype in ntypes}
            aggregate_fn = cfg.aggregate

        constraint = None
        if cfg.max_norm is not None:
            constraint = tf.keras.constraints.max_norm(cfg.max_norm)

        self.ntype_embeddings = {
            ntype: self.add_ntype_embedding(ntype, ntype_embedding_lengths[ntype], ntype_feat_sizes.get(ntype, 0),
                                            constraint, cfg.dropout.input) for ntype in ntypes}
        ...
        def create_module(in_feats, out_feats, name):
            ...
            return GraphConv(in_feats, out_feats, norm=cfg.conv_norm, dropout=cfg.dropout.hidden, constraint=constraint,
        ...
        for layer_i in range(cfg.depth - 1, -1, -1):
