    ...
    with joblib.parallel_backend(cfg.parallel.backend, n_jobs=cfg.parallel.n_jobs), tf.device(cfg.tf.device):
        ...
        def loop_on_problem(problem, ss):
            ...
            try:
                ...
                with trange(cfg.loop_iterations, desc=problem_name, disable=not get_verbose()) as t:
                    for i in t:
                        ...
                        if i == 0:
                            ...
                            def process(proof_search):
                                ...
                                false_negatives = {k: v for k, v in proof_search[False].items() if
                                                   k in proof_feature_vector_indices}
                                    ...
                                    for feature_vector_hash, v in p.items():
                                        ...
                                        for role_proof in [False, True]:
                                            if v['role_proof'][role_proof] == 0:
                                                continue
                                            proof_search[role_proof][feature_vector_index] += v['role_proof'][
                                                role_proof]
                        ...
                        if problem_weights is not None:
                            ...
                            header = ','.join(
                                [k for k in weight_dict.keys() if k != 'symbol'] + list(weight_dict['symbol'].keys())[
                                                                                   1:])
                            np.savetxt(os.path.join(problem_dir, 'weights.csv'), np.concatenate(all_weights),
        ...
        if get_verbose():
            print(f'Looping on {len(problem_paths_all)} problems...', file=sys.stderr)
        with get_parallel(len(problem_paths_all)) as parallel:
            results = parallel(joblib.delayed(loop_on_problem)(p, seed) for p, seed in
                               zip(problem_paths_all, ss.spawn(len(problem_paths_all))))
        records = []
